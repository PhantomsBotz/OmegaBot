...
module.exports = {
  name: 'enable',
  description: 'Enables a command or entire selection of commands',
  category: 'Staff',
...
let command = bot.commands.get(args[0].toLowerCase());
  ...
  con.query(`SELECT ${args[0]} FROM server`, function (err, result, fields) {
    if (err) throw err;
    if (result === '0') return message.channel.send('That command is enabled already!')
    else {
      let sql = `UPDATE server SET ${args[0]} = '0' WHERE guildid = ${message.guild.id}`;
